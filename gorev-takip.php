...
if(!in_array($tab, ['gorev','personel','rapor'], true)){
    $tab = 'gorev';
}
...
try {
    if(($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST'){
        ...
        if($action === 'gorev_kaydet'){
            ...
            if($id > 0){
                $q = $db->prepare("UPDATE gorevler SET personel_id=?, baslik=?, aciklama=?, baslangic_tarihi=?, bitis_tarihi=?, durum=?, tamamlanma_tarihi=? WHERE id=?");
                $q->execute([$personelId, $baslik, $aciklama, $baslangic ?: null, $bitis ?: null, $durum, $tamamlanma, $id]);
                $message = 'Görev güncellendi.';
            }else{
                $q = $db->prepare("INSERT INTO gorevler (personel_id,baslik,aciklama,baslangic_tarihi,bitis_tarihi,durum,link_token,created_by) VALUES (?,?,?,?,?,?,?,?)");
                $q->execute([$personelId, $baslik, $aciklama, $baslangic ?: null, $bitis ?: null, $durum, task_token(), (int)$_SESSION['user_id']]);
...
<div class="main">
    ...
    <div class="panel">
        ...
        <form method="POST" class="grid">
            ...
            <div class="field"><label>Başlangıç</label><input type="date" name="baslangic_tarihi" value="<?php echo task_e($editingTask['baslangic_tarihi'] ?? date('Y-m-d')); ?>"></div>
            <div class="field"><label>Bitiş / Süre</label><input type="date" name="bitis_tarihi" value="<?php echo task_e($editingTask['bitis_tarihi'] ?? ''); ?>"></div>
    ...
    <div class="layout">
        <div class="panel">
            ...
            <div class="table-wrap"><table><thead><tr><th>Görev</th><th>Personel</th><th>Süre</th><th>Durum</th><th>Link / E-posta</th><th>İşlem</th></tr></thead><tbody>
            <?php if(!$gorevler): ?><tr><td colspan="6"><div class="empty">Henüz görev yok.</div></td></tr><?php endif; ?>
