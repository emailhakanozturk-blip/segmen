...
{
    return [
        'baslamadi' => 'HiÃƒÂ§ BaÃ…Å¸lamadÃ„Â±',
        'devam' => 'Devam Ediyor',
        'tamamlandi' => 'TamamlandÃ„Â±',
    ][$status] ?? 'HiÃƒÂ§ BaÃ…Å¸lamadÃ„Â±';
}
...
if(!$canManageTasks && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' || isset($_GET['edit']) || isset($_GET['personel_edit']) || isset($_GET['toplanti_edit']))){
    ...
    exit('Bu iÃ…Å¸lem iÃƒÂ§in yetkiniz yok.');
}
    ...
    if(($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST'){
        ...
        if($action === 'toplanti_kaydet'){
            ...
            if($id > 0){
                ...
                $message = 'ToplantÃ„Â± tutanaÃ„Å¸Ã„Â± oluÃ…Å¸turuldu. Ã…Âimdi bu toplantÃ„Â±dan gÃƒÂ¶rev atayabilirsiniz.';
            }
            ...
            if($atanan > 0){
                $message .= ' ' . $atanan . ' gÃƒÂ¶rev atandÃ„Â±.';
            }
            $tab = 'toplanti';
        }
        ...
        if($action === 'toplanti_sil'){
            ...
            $message = 'ToplantÃ„Â± tutanaÃ„Å¸Ã„Â± silindi.';
            $tab = 'toplanti';
        }
        ...
        if($action === 'personel_kaydet'){
            ...
            if($id > 0){
                ...
                $message = 'Personel gÃƒÂ¼ncellendi.';
            }else{
        ...
        if($action === 'gorev_kaydet'){
            ...
            if($personelId <= 0 || $baslik === ''){
                throw new Exception('Personel ve gÃƒÂ¶rev baÃ…Å¸lÃ„Â±Ã„Å¸Ã„Â± zorunludur.');
            }
        ...
        if($action === 'email_gonder'){
            ...
            if(!$task || trim((string)$task['email']) === ''){
                throw new Exception('GÃƒÂ¶reve ait personel e-postasÃ„Â± yok.');
            }
            ...
            $subject = 'Yeni gÃƒÂ¶rev: ' . $task['baslik'];
            $body = "Merhaba {$task['ad_soyad']},\n\nSize bir gÃƒÂ¶rev atandÃ„Â±.\n\nGÃƒÂ¶rev: {$task['baslik']}\nBitiÃ…Å¸: " . task_date($task['bitis_tarihi']) . "\n\nGÃƒÂ¶revi gÃƒÂ¶rÃƒÂ¼ntÃƒÂ¼lemek iÃƒÂ§in:\n{$link}";
            $sent = @mail((string)$task['email'], $subject, $body, "Content-Type: text/plain; charset=UTF-8\r\n");
            $message = $sent ? 'GÃƒÂ¶rev e-postasÃ„Â± gÃƒÂ¶nderildi.' : 'Sunucu mail gÃƒÂ¶nderimini kabul etmedi. Linki kopyalayarak gÃƒÂ¶nderebilirsiniz: ' . $link;
            $tab = 'gorev';
        }
    }
} catch(Throwable $e){
...
if(isset($_GET['toplanti_export'])){
    ...
    if(!$exportToplanti){
        ...
        exit('ToplantÃ„Â± tutanaÃ„Å¸Ã„Â± bulunamadÃ„Â±.');
    }
    ...
    if(!$canManageTasks && !$exportGorevler){
        http_response_code(403);
        exit('Bu toplantÃƒâ€Ã‚Â± tutanaÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±nÃƒâ€Ã‚Â± gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼ntÃƒÆ’Ã‚Â¼leme yetkiniz yok.');
    }
...
<div class="main">
    <div class="topbar">
        <h2>GÃƒÂ¶rev Takip</h2>
        <p>Personele gÃƒÂ¶rev atayÃ„Â±n, sÃƒÂ¼re verin, durumlarÃ„Â± ve geciken iÃ…Å¸leri tek ekranda izleyin.</p>
    </div>
    ...
    <nav class="task-tabs">
        <a class="task-tab <?php echo $tab==='toplanti'?'active':''; ?>" href="?tab=toplanti">ToplantÃ„Â± TutanaklarÃ„Â±</a>
        <a class="task-tab <?php echo $tab==='gorev'?'active':''; ?>" href="?tab=gorev">GÃƒÂ¶revler</a>
    ...
    <div class="layout">
        <div class="panel">
            <h3><?php echo $editingToplanti ? 'ToplantÃ„Â± TutanaÃ„Å¸Ã„Â± DÃƒÂ¼zenle' : 'ToplantÃ„Â± TutanaÃ„Å¸Ã„Â± OluÃ…Å¸tur'; ?></h3>
            <form method="POST" class="grid">
                ...
                <div class="field"><label>ToplantÃ„Â± GÃƒÂ¼nÃƒÂ¼</label><input type="date" name="toplanti_tarihi" required value="<?php echo task_e($editingToplanti['toplanti_tarihi'] ?? date('Y-m-d')); ?>"></div>
                <div class="field span-2"><label>ToplantÃ„Â± BaÃ…Å¸lÃ„Â±Ã„Å¸Ã„Â±</label><input name="baslik" required value="<?php echo task_e($editingToplanti['baslik'] ?? ''); ?>" placeholder="HaftalÃ„Â±k operasyon toplantÃ„Â±sÃ„Â±"></div>
                <div class="field span-4"><label>ToplantÃ„Â± TutanaÃ„Å¸Ã„Â±</label><textarea id="meetingNotes" name="tutanak" placeholder="Her maddeyi ayrÃ„Â± satÃ„Â±ra yazÃ„Â±n."><?php echo task_e($editingToplanti['tutanak'] ?? ''); ?></textarea></div>
                <div class="span-4 minute-assign-box">
                    <b>Tutanak Ã„Â°ÃƒÂ§inden GÃƒÂ¶rev Ata</b>
                    <p class="muted">Her satÃ„Â±r ayrÃ„Â± gÃƒÂ¶rev maddesi olur. Personel seÃƒÂ§erseniz toplantÃ„Â± kaydÃ„Â±yla birlikte gÃƒÂ¶rev aÃƒÂ§Ã„Â±lÃ„Â±r.</p>
                    <select id="personelOptionsSource" style="display:none"><option value="">GÃƒÂ¶rev atama</option><?php foreach($aktifPersoneller as $p): ?><option value="<?php echo (int)$p['id']; ?>"><?php echo task_e($p['ad_soyad']); ?><?php echo $p['unvan'] ? ' - ' . task_e($p['unvan']) : ''; ?></option><?php endforeach; ?></select>
                    <div id="minuteAssignList"></div>
                </div>
                <div class="span-4 actions"><button class="btn btn-green"><?php echo $editingToplanti ? 'GÃƒÂ¼ncelle' : 'Tutanak OluÃ…Å¸tur'; ?></button><?php if($editingToplanti): ?><a class="btn btn-gray" href="?tab=toplanti">VazgeÃƒÂ§</a><?php endif; ?></div>
            </form>
        </div>
        <aside class="panel timeline">
            <h3>ToplantÃ„Â±dan GÃƒÂ¶rev Atama</h3>
            <p class="empty">Bir tutanak oluÃ…Å¸turduktan sonra listeden Ã¢â‚¬Å“GÃƒÂ¶rev AtaÃ¢â‚¬Â seÃƒÂ§in. GÃƒÂ¶rev formunda toplantÃ„Â± ve gÃƒÂ¼n otomatik gelir.</p>
        </aside>
    </div>
    <?php endif; ?>
    <div class="panel">
        <h3>ToplantÃ„Â± TutanaklarÃ„Â±</h3>
        <div class="table-wrap"><table><thead><tr><th>GÃƒÂ¼n</th><th>ToplantÃ„Â±</th><th>Tutanak</th><th>GÃƒÂ¶rev</th><th>Ã„Â°Ã…Å¸lem</th></tr></thead><tbody>
        <?php if(!$toplantilar): ?><tr><td colspan="5">HenÃƒÂ¼z toplantÃ„Â± tutanaÃ„Å¸Ã„Â± yok.</td></tr><?php endif; ?>
        <?php foreach($toplantilar as $t): ?><tr>
            ...
            <td><span class="badge work"><?php echo (int)$t['gorev_adet']; ?> gÃƒÂ¶rev</span></td>
            <td><div class="actions"><?php if($canManageTasks): ?><a class="btn btn-green" href="?tab=gorev&toplanti_id=<?php echo (int)$t['id']; ?>">GÃƒÂ¶rev Ata</a><?php endif; ?><a class="btn btn-dark" target="_blank" href="?toplanti_export=<?php echo (int)$t['id']; ?>&format=pdf">PDF</a><a class="btn btn-gray" href="?toplanti_export=<?php echo (int)$t['id']; ?>&format=word">Word</a><?php if($canManageTasks): ?><a class="btn" href="?tab=toplanti&toplanti_edit=<?php echo (int)$t['id']; ?>">DÃƒÂ¼zelt</a><form method="POST" onsubmit="return confirm('ToplantÃ„Â± tutanaÃ„Å¸Ã„Â± silinsin mi? GÃƒÂ¶revler silinmez, toplantÃ„Â± baÃ„Å¸lantÃ„Â±sÃ„Â± kaldÃ„Â±rÃ„Â±lÃ„Â±r.');"><input type="hidden" name="action" value="toplanti_sil"><input type="hidden" name="id" value="<?php echo (int)$t['id']; ?>"><button class="btn btn-red">Sil</button></form><?php endif; ?></div></td>
    ...
    <div class="summary">
        ...
        <div class="summary-card"><span>HiÃƒÂ§ BaÃ…Å¸lamadÃ„Â±</span><strong><?php echo (int)($summary['baslamadi'] ?? 0); ?></strong></div>
    ...
    <div class="panel">
        <h3><?php echo task_date($selectedToplanti['toplanti_tarihi']); ?> ToplantÃ„Â± Maddeleri</h3>
        <?php if(!$toplantiMaddeleri): ?><div class="empty">Bu tutanakta madde yok. Tutanak metnini her madde ayrÃ„Â± satÃ„Â±r olacak Ã…Å¸ekilde yazabilirsiniz.</div><?php endif; ?>
        <div class="minute-list">
            <?php foreach($toplantiMaddeleri as $madde): ?>
                <div class="minute-row"><span><?php echo task_e($madde); ?></span><a class="btn btn-green" href="?tab=gorev&toplanti_id=<?php echo (int)$selectedToplanti['id']; ?>&madde=<?php echo urlencode($madde); ?>">Bu Maddeden GÃƒÂ¶rev Ata</a></div>
            <?php endforeach; ?>
        </div>
    </div>
    <?php endif; ?>
    ...
    <?php if($canManageTasks): ?>
    <div class="panel">
        <h3><?php echo $editingTask ? 'GÃƒÂ¶rev DÃƒÂ¼zenle' : 'Yeni GÃƒÂ¶rev'; ?></h3>
        <form method="POST" class="grid">
            <input type="hidden" name="action" value="gorev_kaydet">
            <input type="hidden" name="id" value="<?php echo (int)($editingTask['id'] ?? 0); ?>">
            <div class="field span-2"><label>ToplantÃ„Â± TutanaÃ„Å¸Ã„Â±</label><select name="toplanti_id"><option value="">BaÃ„Å¸Ã„Â±msÃ„Â±z gÃƒÂ¶rev</option><?php foreach($toplantilar as $t): ?><option value="<?php echo (int)$t['id']; ?>" <?php echo $selectedToplantiId===(int)$t['id']?'selected':''; ?>><?php echo task_date($t['toplanti_tarihi']); ?> - <?php echo task_e($t['baslik']); ?></option><?php endforeach; ?></select></div>
            <div class="field span-2"><label>Personel</label><select name="personel_id" required><option value="">SeÃƒÂ§iniz</option><?php foreach($aktifPersoneller as $p): ?><option value="<?php echo (int)$p['id']; ?>" <?php echo (int)($editingTask['personel_id'] ?? 0)===(int)$p['id']?'selected':''; ?>><?php echo task_e($p['ad_soyad']); ?><?php echo $p['unvan'] ? ' - ' . task_e($p['unvan']) : ''; ?></option><?php endforeach; ?></select></div>
            <div class="field span-2"><label>GÃƒÂ¶rev BaÃ…Å¸lÃ„Â±Ã„Å¸Ã„Â±</label><input name="baslik" required value="<?php echo task_e($defaultTitle); ?>"></div>
            <div class="field"><label>GÃƒÂ¶rev GÃƒÂ¼nÃƒÂ¼</label><input type="date" name="baslangic_tarihi" value="<?php echo task_e($defaultStartDate); ?>"></div>
            <div class="field"><label>BitiÃ…Å¸ / SÃƒÂ¼re</label><input type="date" name="bitis_tarihi" value="<?php echo task_e($editingTask['bitis_tarihi'] ?? $defaultStartDate); ?>"></div>
            ...
            <div class="field span-4"><label>AÃƒÂ§Ã„Â±klama</label><textarea name="aciklama"><?php echo task_e($defaultDescription); ?></textarea></div>
            <div class="span-4 actions"><button class="btn btn-green"><?php echo $editingTask ? 'GÃƒÂ¼ncelle' : 'GÃƒÂ¶rev OluÃ…Å¸tur'; ?></button><?php if($editingTask): ?><a class="btn btn-gray" href="?tab=gorev">VazgeÃƒÂ§</a><?php endif; ?></div>
        </form>
    </div>
    <?php endif; ?>
    ...
    <div class="layout">
        <div class="panel">
            <form method="GET" class="filters">
                ...
                <div class="field"><label>Personel</label><select name="personel"><option value="">TÃƒÂ¼mÃƒÂ¼</option><?php foreach($personeller as $p): ?><option value="<?php echo (int)$p['id']; ?>" <?php echo $personelFiltre===(int)$p['id']?'selected':''; ?>><?php echo task_e($p['ad_soyad']); ?></option><?php endforeach; ?></select></div>
                <div class="field"><label>Durum</label><select name="durum"><option value="">TÃƒÂ¼mÃƒÂ¼</option><?php foreach(['baslamadi','devam','tamamlandi'] as $d): ?><option value="<?php echo $d; ?>" <?php echo $durumFiltre===$d?'selected':''; ?>><?php echo task_status_label($d); ?></option><?php endforeach; ?></select></div>
                <button class="btn">Filtrele</button><a class="btn btn-gray" href="?tab=gorev">Temizle</a>
            </form>
            <div class="table-wrap"><table><thead><tr><th>ToplantÃ„Â±</th><th>GÃƒÂ¶rev</th><th>Personel</th><th>SÃƒÂ¼re</th><th>Durum</th><th>Link / E-posta</th><th>Ã„Â°Ã…Å¸lem</th></tr></thead><tbody>
            <?php if(!$gorevler): ?><tr><td colspan="7"><div class="empty">HenÃƒÂ¼z gÃƒÂ¶rev yok.</div></td></tr><?php endif; ?>
            <?php foreach($gorevler as $g): $cls=task_status_class($g); $link='gorev-goruntule.php?token='.urlencode($g['link_token']); ?>
            <tr>
                <td><?php echo $g['toplanti_baslik'] ? '<strong>'.task_e($g['toplanti_baslik']).'</strong><br><small>'.task_date($g['toplanti_tarihi']).'</small>' : '<span class="muted">BaÃ„Å¸Ã„Â±msÃ„Â±z</span>'; ?></td>
                ...
                <td><div class="actions"><?php if($canManageTasks): ?><a class="btn" href="?tab=gorev&edit=<?php echo (int)$g['id']; ?>">DÃƒÂ¼zelt</a><form method="POST" onsubmit="return confirm('GÃƒÂ¶rev silinsin mi?');"><input type="hidden" name="action" value="gorev_sil"><input type="hidden" name="id" value="<?php echo (int)$g['id']; ?>"><button class="btn btn-red">Sil</button></form><?php else: ?><span class="muted">Sadece gÃƒÂ¶rÃƒÂ¼ntÃƒÂ¼leme</span><?php endif; ?></div></td>
            </tr>
            <?php endforeach; ?></tbody></table></div>
        </div>
        <aside class="panel timeline">
            <h3>Zaman Ãƒâ€¡izelgesi</h3>
            <div class="timeline-list">
                ...
                <?php endforeach; ?>
                <?php if(!$gorevler): ?><div class="empty">Ãƒâ€¡izelgede gÃƒÂ¶sterilecek gÃƒÂ¶rev yok.</div><?php endif; ?>
            </div>
        </aside>
    </div>
    <?php endif; ?>
    ...
    <?php if($canManageTasks): ?>
    <div class="panel">
            <h3><?php echo $editingPersonel ? 'Personel DÃƒÂ¼zenle' : 'Personel Ekle'; ?></h3>
            <form method="POST" class="grid">
                <input type="hidden" name="action" value="personel_kaydet"><input type="hidden" name="id" value="<?php echo (int)($editingPersonel['id'] ?? 0); ?>">
                <div class="field span-2"><label>KullanÃ„Â±cÃ„Â±</label><select name="user_id" required><option value="">SeÃƒÂ§iniz</option><?php foreach($usersForTask as $u): ?><option value="<?php echo (int)$u['id']; ?>" <?php echo (int)($editingPersonel['user_id'] ?? 0)===(int)$u['id']?'selected':''; ?>><?php echo task_e($u['name']); ?> - <?php echo task_e($u['email']); ?></option><?php endforeach; ?></select></div>
                ...
                <div class="span-4 actions"><button class="btn btn-green"><?php echo $editingPersonel ? 'GÃƒÂ¼ncelle' : 'Kaydet'; ?></button><?php if($editingPersonel): ?><a class="btn btn-gray" href="?tab=personel">VazgeÃƒÂ§</a><?php endif; ?></div>
            </form>
    </div>
    <?php endif; ?>
    <div class="panel">
            <h3>Personel Listesi</h3>
            <div class="table-wrap"><table><thead><tr><th>Ad Soyad</th><th>KullanÃ„Â±cÃ„Â±</th><th>E-posta</th><th>Durum</th><th>Ã„Â°Ã…Å¸lem</th></tr></thead><tbody>
            <?php foreach($personeller as $p): ?><tr><td><strong><?php echo task_e($p['ad_soyad']); ?></strong><br><small><?php echo task_e($p['unvan']); ?></small></td><td><?php echo task_e($p['user_name'] ?? '-'); ?></td><td><?php echo task_e($p['email']); ?></td><td><span class="badge <?php echo (int)$p['aktif']===1?'done':'wait'; ?>"><?php echo (int)$p['aktif']===1?'Aktif':'Pasif'; ?></span></td><td><div class="actions"><?php if($canManageTasks): ?><a class="btn" href="?tab=personel&personel_edit=<?php echo (int)$p['id']; ?>">DÃƒÂ¼zelt</a><form method="POST" onsubmit="return confirm('Personel silinsin mi?');"><input type="hidden" name="action" value="personel_sil"><input type="hidden" name="id" value="<?php echo (int)$p['id']; ?>"><button class="btn btn-red">Sil</button></form><?php else: ?><span class="muted">Sadece gÃƒÂ¶rÃƒÂ¼ntÃƒÂ¼leme</span><?php endif; ?></div></td></tr><?php endforeach; ?>
            <?php if(!$personeller): ?><tr><td colspan="5">HenÃƒÂ¼z personel yok.</td></tr><?php endif; ?></tbody></table></div>
    </div>
    <?php endif; ?>
    ...
    <div class="report-layout">
        <div class="panel"><h3>Personel BazlÃ„Â± GÃƒÂ¶rev Ãƒâ€“zeti</h3><div class="table-wrap"><table><thead><tr><th>Personel</th><th>BaÃ…Å¸langÃ„Â±ÃƒÂ§</th><th>BitiÃ…Å¸</th><th>Toplam</th><th>HiÃƒÂ§ BaÃ…Å¸lamadÃ„Â±</th><th>Devam</th><th>Tamamlanan</th><th>Geciken</th></tr></thead><tbody>
...
(function(){
    ...
    function renderAssignments(){
        ...
        list.innerHTML = '';
        if(!rows.length){
            list.innerHTML = '<div class="empty">Tutanak satÃ„Â±rÃ„Â± yazÃ„Â±nca burada gÃƒÂ¶rev atama alanlarÃ„Â± gÃƒÂ¶rÃƒÂ¼nÃƒÂ¼r.</div>';
            return;
        }
