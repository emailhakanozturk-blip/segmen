...
$menuGroups = [
    'Genel Bakış' => [['dashboard.php', 'Dashboard', 'grid']],
    'Yönetim' => [['cariler.php', 'Cariler', 'building'], ['sozlesmeler.php', 'Sözleşmeler', 'file'], ['promosyon-sozlesmeleri.php', 'Sponsorluk Sözleşmeleri', 'star'], ['tanimlar.php', 'Tanımlar', 'sliders'], ['nokta-yonetimi.php', 'Nokta Yönetimi', 'pin']],
    'Operasyon' => [['hakedisler.php', 'Hakedişler', 'receipt'], ['motorin-yukle.php', 'Motorin Fiyatları', 'fuel'], ['demirbas-takip.php', 'Demirbaş Takip', 'box'], ['palet-takip.php', 'Palet Takip', 'layers'], ['recete-maliyet.php', 'Reçete ve Maliyet', 'chart'], ['gorev-takip.php', 'Görev Takip', 'check']],
    'Sistem' => [['raporlar.php', 'Raporlar', 'report'], ['log-kayitlari.php', 'Log Kayıtları', 'clock'], ['kullanici-yonetimi.php', 'Ayarlar', 'settings']],
];
$icons = [
...
<aside class="sidebar" aria-label="Ana menü">
    <a class="brand" href="dashboard.php" aria-label="Seğmen Su ana sayfa">
        <img src="assets/img/segmen-su-logo.png" alt="Seğmen Su" class="sidebar-logo-img">
        <span>Hakediş Hesaplama<br>Modülü</span>
    </a>
    <nav class="menu">
            ...
            <div class="menu-group">
                ...
                <?php foreach($items as [$href, $label, $icon]): ?>
                    <a href="<?= $href ?>" class="<?= $activePage === $href ? 'is-active' : '' ?>"<?= $activePage === $href ? ' aria-current="page"' : '' ?>>
                        <svg viewBox="0 0 24 24" aria-hidden="true"><?= $icons[$icon] ?></svg><span><?= htmlspecialchars($label) ?></span>
                    </a>
                <?php endforeach; ?>
            </div>
        <?php endforeach; ?>
    ...
    <div class="sidebar-footer">
        <a class="logout-link" href="logout.php" onclick="return confirm('Çıkış yapılsın mı?');">
            <svg viewBox="0 0 24 24" aria-hidden="true"><?= $icons['logout'] ?></svg><span>Güvenli Çıkış</span>
        </a>
    </div>
</aside>
